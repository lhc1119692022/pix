...
export function runTar(args, opts = {}) {
  ...
  if (process.platform === "win32") {
    try {
      // Windows' built-in bsdtar accepts native drive paths, but not MSYS /c paths.
      execFileSync("tar", args, { stdio });
      return;
    } catch {
      // Git for Windows GNU tar needs MSYS paths to avoid treating C: as a host.
    }
  }
