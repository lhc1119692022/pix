...
describe("chip labels", () => {
  ...
  it("uses a chip face when the label is shorter than the raw token", () => {
    expect(shouldUseChipFace({ kind: "skill", text: "/skill:review", label: "review" })).toBe(true);
    expect(shouldUseChipFace({ kind: "slash", text: "/new", label: "new" })).toBe(false);
    expect(shouldUseChipFace({ kind: "url", text: "https://example.com" })).toBe(false);
  });
